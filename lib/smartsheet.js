...
// Question ids unique to one build. Used to infer the audience if neither
// the client nor the server reported one.
...
function resolveAudience(answers = {}, reported) {
  const claimed = String(reported || "").toLowerCase();
  if (claimed === "internal") return "Internal";
  if (claimed === "external") return "External";

  if (AUDIENCE === "internal") return "Internal";
  if (AUDIENCE === "external") return "External";

  const keys = Object.keys(answers);
  if (keys.some((k) => EXTERNAL_ONLY_IDS.includes(k))) return "External";
  if (keys.some((k) => INTERNAL_ONLY_IDS.includes(k))) return "Internal";
  return "Unknown";
}

export async function submitToSmartsheet({ answers, submittedAt, audience }) {
  ...
  //   1. What the client reported. That is the build that actually served
  //      the questions, so it cannot disagree with what was on screen.
  //   2. This server bundle's own AUDIENCE.
  //   3. Inferred from which question ids are present. External-only ids
  //      cannot appear in an internal submission and vice versa, so this
  //      is a reliable last resort.
  if (columns[METADATA_COLUMNS.audience]) {
    cells.push({
      columnId: columns[METADATA_COLUMNS.audience],
      value: resolveAudience(answers, audience),
    });
  }
